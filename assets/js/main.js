...
document.addEventListener('DOMContentLoaded', function () {
  ...
  wholecookie.addEventListener('click', () => {
    ...
    if (fortuneText) {
      getRandomProverb().then(proverb => {
        fortuneText.textContent = proverb;
        fortuneText.style.display = 'inline';
      });
    }
...
async function getRandomProverb() {
  try {
    const response = await fetch('assets/fortunes/proverbs.txt');
    ...
    console.error('Error fetching proverbs file:', error);
